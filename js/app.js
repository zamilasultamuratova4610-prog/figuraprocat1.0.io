...
// ═══════════════════════════════════════════
// PDF + ШАРИНГ (работает в WebView/APK)
// ═══════════════════════════════════════════
async function exportPDF(){
  ...
  try{
    ...
    function drawCard(d,x,y){
      ...
      const[fr2,fg2,fb2]=h2r(sc.fg);doc.setTextColor(fr2,fg2,fb2);sfb(5.5);doc.text(sc.l,tx+2,ty);ty+=6.5;
    ...
    pb.style.width="100%";msg.textContent="Готово!";await new Promise(r=>setTimeout(r,300));
    document.getElementById("pdfOvl").style.display="none";

    // ★★★ СОХРАНЯЕМ PDF КАК DATA URI (работает в WebView) ★★★
    const dataUri=doc.output("datauristring");
    window._pdfDataUri=dataUri;
    window._pdfTotal=total;
    window._pdfAvail=avail;

    // Показываем модалку с вариантами
    showPDFModal(total,avail);
    showToast("PDF создан!");
...
// ═══════════════════════════════════════════
// ★ МОДАЛКА С ВАРИАНТАМИ ОТПРАВКИ
// ═══════════════════════════════════════════
function showPDFModal(total,avail){
  document.getElementById("moTitle").textContent="📄 Каталог готов!";
  document.getElementById("moBox").classList.remove("wide");
  document.getElementById("mov").style.display="flex";
  document.getElementById("moBody").innerHTML=
    '<div class="bsuc">'+
      '<div class="bsuc-i">✅</div>'+
      '<h3>Каталог сформирован!</h3>'+
      '<p style="margin-bottom:20px">'+total+' костюмов, свободно: '+avail+'</p>'+

      '<button onclick="pdfOpen()" style="display:flex;align-items:center;justify-content:center;gap:8px;width:100%;max-width:300px;margin:0 auto 10px;padding:14px 20px;background:linear-gradient(135deg,#0ea5e9,#6366f1);color:#fff;border:none;border-radius:14px;font-size:14px;font-weight:700;cursor:pointer">'+
        '📄 Открыть / Скачать PDF'+
      '</button>'+

      '<button onclick="pdfShareText()" style="display:flex;align-items:center;justify-content:center;gap:8px;width:100%;max-width:300px;margin:0 auto 10px;padding:14px 20px;background:#25D366;color:#fff;border:none;border-radius:14px;font-size:14px;font-weight:700;cursor:pointer">'+
        '💬 Отправить в WhatsApp'+
      '</button>'+

      '<button onclick="pdfCopyText()" style="display:flex;align-items:center;justify-content:center;gap:8px;width:100%;max-width:300px;margin:0 auto 10px;padding:14px 20px;background:var(--g100);color:var(--g700);border:none;border-radius:14px;font-size:14px;font-weight:700;cursor:pointer">'+
        '📋 Копировать каталог текстом'+
      '</button>'+

      '<div style="background:var(--g50);border-radius:12px;padding:12px;margin-top:16px;text-align:left">'+
        '<div style="font-size:12px;font-weight:700;margin-bottom:6px">💡 Как отправить клиенту:</div>'+
        '<div style="font-size:11px;color:var(--g500);line-height:1.6">'+
          '1. Нажмите «Открыть PDF» — файл откроется<br>'+
          '2. Нажмите ⋮ или «Поделиться» в просмотрщике<br>'+
          '3. Выберите WhatsApp или другой мессенджер<br>'+
          '<br>Или нажмите «Копировать текстом» и вставьте в чат'+
        '</div>'+
      '</div>'+

      '<button onclick="closeModal()" class="bcn" style="max-width:200px;margin:16px auto 0;display:block">Закрыть</button>'+
    '</div>';
}

// ═══════════════════════════════════════════
// ★ ОТКРЫТЬ PDF (работает в WebView!)
// ═══════════════════════════════════════════
function pdfOpen(){
  if(!window._pdfDataUri){showToast("Сначала создайте PDF","error");return}
  try{
    // Способ 1: открываем data URI в новом окне
    // В WebView Android это вызовет системный просмотрщик PDF
    var w=window.open(window._pdfDataUri,"_blank");
    if(w){
      showToast("PDF открыт!");
      return;
    }
  }catch(e){}

  try{
    // Способ 2: переход по data URI — принудительное скачивание
    var link=document.createElement("a");
    link.href=window._pdfDataUri;
    link.download="FiguraProkat.pdf";
    link.target="_blank";
  ...
  try{
    ...
    window.location.href=window._pdfDataUri;
    showToast("PDF открывается...");
  }catch(e){
...
// ═══════════════════════════════════════════
// ★ ОТПРАВИТЬ В WHATSAPP (текст каталога)
// ═══════════════════════════════════════════
function pdfShareText(){
  var text=buildCatalogText();

  // Пробуем Web Share API (текст — работает везде)
  if(navigator.share){
    navigator.share({
      title:"ФигураПрокат — Каталог",
      text:text
    }).then(function(){
      showToast("Отправлено!");
    }).catch(function(err){
      if(err.name!=="AbortError"){
        // Fallback на WhatsApp ссылку
        openWhatsApp(text);
      }
    });
  }else{
...
function showTextModal(text){
  document.getElementById("moTitle").textContent="📋 Каталог текстом";
  document.getElementById("moBody").innerHTML=
    '<p style="font-size:12px;color:var(--g500);margin-bottom:10px">Выделите текст и скопируйте:</p>'+
    '<textarea class="fta" rows="15" style="font-size:11px;line-height:1.5" readonly onclick="this.select()">'+text+'</textarea>'+
    '<div class="fbtns"><button class="bcn" onclick="closeModal()">Закрыть</button></div>';
}

// ═══════════════════════════════════════════
// ★ ГЕНЕРАЦИЯ ТЕКСТА КАТАЛОГА
// ═══════════════════════════════════════════
function buildCatalogText(){
  var list=getClFiltered();
  var avail=list.filter(function(d){return gDS(d.id)==="available"}).length;
  var lines=[];

  lines.push("⛸️ *ФИГУРАПРОКАТ*");
  lines.push("━━━━━━━━━━━━━━━");
  lines.push("📅 "+new Date().toLocaleDateString("ru-RU"));
  lines.push("👗 Костюмов: "+list.length+" | ✅ Свободно: "+avail);
  lines.push("");

  list.forEach(function(d,i){
    var s=gDS(d.id);
    var icon=s==="available"?"✅":s==="booked"?"🕐":s==="rented"?"🚫":"🔄";
    var stLabel=ST[s].l;

    lines.push("━━━━━━━━━━━━━━━");
    lines.push(icon+" *"+(i+1)+". "+d.name+"*");
    lines.push("   "+d.category+" | "+d.style);
    lines.push("   📏 "+d.size+" | "+d.material);
    if(d.decoration&&d.decoration!=="Без декора"){
      lines.push("   ✨ "+d.decoration);
    }
    lines.push("   💰 *"+fmt(d.price)+"*/сут");
    lines.push("   🔒 Залог: "+fmt(d.deposit));
    lines.push("   📌 "+stLabel);

    // Даты брони
    var bookings=getDressBookings(d.id);
    if(bookings.length){
      bookings.forEach(function(bk){
        var bIcon=bk.status==="rented"?"🚫":"🕐";
        lines.push("   "+bIcon+" "+fmtDs(bk.start)+"-"+fmtDs(bk.end)+" ("+bk.client+")");
      });
    }
  ...
  lines.push("━━━━━━━━━━━━━━━");
  lines.push("📞 Для бронирования свяжитесь с нами!");
  lines.push("⛸️ ФигураПрокат");
